...
object PlyReader {
  ...
  private def loadSpecifiedTextures(header: List[String], dir: Path): List[PixelImage[RGBA]] = {
    ...
    val linesWithTexture = header.filter(s => s.startsWith(PLY.comment + " " + PLY.textureFile))
    ...
    val textureURLs = textureFiles.map { fn =>
      val filename = dir.resolve(Paths.get(fn))
      if (!Files.exists(filename)) {
        throw new IOException("Could not find texture file \"" + fn + "\"")
      } else {
        filename
      }
    }
