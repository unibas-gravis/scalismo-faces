...
class CorrespondenceMoMoRenderer(override val model: MoMo, override val clearColor: RGBA)
    extends MoMoRenderer(model, clearColor) {
  ...
  def renderCorrespondenceImage(parameters: RenderParameter): PixelImage[Option[TriangleFragment]] = {
    ...
    TriangleRenderer
      .renderCorrespondence(inst.shape,
                            backfaceCullingFilter,
                            parameters.pointShader,
                            parameters.imageSize.screenTransform,
                            buffer
      )
      .toImage
  }
  ...
  override def cached(cacheSize: Int) = new CorrespondenceMoMoRenderer(model, clearColor) {
    ...
    override def renderCorrespondenceImage(parameters: RenderParameter): PixelImage[Option[TriangleFragment]] =
      correspondenceImageRenderer(parameters)
...
abstract class RenderFromCorrespondenceImage[A](correspondenceMoMoRenderer: CorrespondenceMoMoRenderer)
    extends ParametricImageRenderer[A] {
  override def renderImage(parameters: RenderParameter): PixelImage[A]
}

case class CorrespondenceColorImageRenderer(correspondenceMoMoRenderer: CorrespondenceMoMoRenderer,
                                            backgroundColor: RGBA = RGBA.Black
) extends RenderFromCorrespondenceImage[RGBA](correspondenceMoMoRenderer) {
  ...
  val normalizedReference: SurfacePointProperty[RGBA] = {
    ...
    val normalizedPoints = reference.shape.pointSet.points
      .map(p => (p.toBreezeVector - min) /:/ extent)
      .map(f => EuclideanVector[_3D](f.toArray))
      .toIndexedSeq
