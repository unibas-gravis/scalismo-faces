...
import scalismo.geometry.{_1D, _2D, Point, Point1D}
import scalismo.sampling.DistributionEvaluator
import scalismo.sampling.evaluators.{PairEvaluator, ProductEvaluator}

/**
 * The LandmarkMapEvaluator evaluates landmark positions by a simple look-up in a LandmarkDetectionMap. These maps
 * usually include a noise-model through precomputation.
 *
 * @param detectionMap
 ...
 */
case class LandmarkMapEvaluator(detectionMap: LandmarkDetectionMap, renderer: ParametricLandmarksRenderer)
...
object LandmarkMapEvaluator {
  ...
  /**
   * Prepare landmarks detection maps with an isotropic Gaussian noise model, combination in a ProductEvaluator
  ...
  /**
   * Prepare landmarks detection maps with the specified noise model, combination in a ProductEvaluator (independent
   * landmarks) WARNING: uses a slow full maximum search internally, use only for exotic noise models The resulting map
   * evaluator is lazy - it might still work efficiently if you do not have too many evaluations
   */
  def withLandmarksLikelihood(detectionMaps: Seq[LandmarkDetectionMap],
                              renderer: ParametricLandmarksRenderer,
                              landmarksLikelihood: PairEvaluator[Point[_2D]]
  ): ProductEvaluator[RenderParameter] = {
    val mapEvaluators = detectionMaps.map { detectionMap =>
      // cache all computations
      val detectionMapWithNoise = PixelImage.view(
        detectionMap.logValues.domain,
        { (x, y) =>
