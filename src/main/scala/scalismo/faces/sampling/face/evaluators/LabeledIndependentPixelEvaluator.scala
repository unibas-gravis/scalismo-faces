...
/**
 * evaluate each pixel independently, uses alpha channel to determine between foreground, background and label to
 * destinguish between face and occlusion/non-face.
 *
 * The face region is labeled as 1 (according to the following publication)
 *
 * This evaluator is an implementation of Equation 2, 4, 5 and 7 of: Occlusion-aware 3D Morphable Face Models, Bernhard
 * Egger, Andreas Schneider, Clemens Blumer, Andreas Morel-Forster, Sandro Schönborn, Thomas Vetter IN: British Machine
 * Vision Conference (BMVC), September 2016 https://dx.doi.org/10.5244/C.30.64
 */
class LabeledIndependentPixelEvaluator(val reference: PixelImage[RGBA],
                                       val pixelEvaluator: PairEvaluator[RGB],
  ...
  override def logValue(sample: LabeledPixelImage[RGBA]): Double = {
    require(sample.label.domain == reference.domain,
            "LabeledIndependentPixelEvaluator: images must be comparable! (different sizes)"
    )
...
object LabeledIndependentPixelEvaluator {
  def apply(reference: PixelImage[RGBA], pixelEvaluator: PairEvaluator[RGB], bgEvaluator: DistributionEvaluator[RGB]) =
    new LabeledIndependentPixelEvaluator(reference, pixelEvaluator, bgEvaluator)
}
