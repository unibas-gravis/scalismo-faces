...
object PreconditionedConjugateGradient {
   ...
   *   solves Mx = b (x = preconditionSolver(b)), more efficient inv(M)*b (where is a crude but accessible approximation
   *   to A)
   */
  def solveSparse(A: CSCMatrix[Double],
                  b: DenseVector[Double],
    ...
    val solver =
      pcgIterator(A, b, preconditioner, initial).take(maxIter).dropWhile(state => norm(state.r) > tolerance).take(1)
    val solution = if (solver.hasNext) solver.next() else initial
    solution.x
  }

  /** create an initial state with an initial guess */
  def initialState(A: CSCMatrix[Double],
                   b: DenseVector[Double],
  ...
  def pcgIterator(A: CSCMatrix[Double],
                  b: DenseVector[Double],
                  preconditioner: DenseVector[Double] => DenseVector[Double],
                  initial: PCGState
  ): Iterator[PCGState] = {
  ...
  def pcgIteration(A: CSCMatrix[Double],
                   b: DenseVector[Double],
                   preconditioner: DenseVector[Double] => DenseVector[Double],
                   state: PCGState
  ): PCGState = {
