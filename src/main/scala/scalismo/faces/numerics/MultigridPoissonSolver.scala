...
object MultigridParameters {
  ...
  val default = MultigridParameters(vCycles = 5,
                                    minSize = 8,
...
/**
 * Multigrid algorithm to solve Poisson's equation in the image domain for generic pixel type A (efficient but may not
 * fully converge with complicated boundaries)
 */
class GenericMultigridPoissonSolver[A: ClassTag](parameters: MultigridParameters = MultigridParameters.default)(implicit
  ...
  /**
   * solve the equation: Laplace(f) = rhs, f|fixedMask = boundary, single level relaxations: slow and does not solve
   * "low frequency part"! use solveMultigridPoisson instead
   */
  ...
  def residual(f: PixelImage[A],
    ...
    PixelImage(
      ...
      (x, y) => {
        if (mask(x, y))
          rhs(x, y) - (image(x - 1, y) + image(x + 1, y)) / dxSq - (image(x, y - 1) + image(x,
                                                                                            y + 1
          )) / dySq + (2.0 / dxSq + 2.0 / dySq) *: image(x, y)
        else
          ops.zero
      },
      f.accessMode
  ...
  def jacobiRelaxation(f: PixelImage[A],
                       mask: PixelImage[Boolean],
                       rhs: PixelImage[A],
                       hx: Double,
                       hy: Double,
    ...
    PixelImage(
      ...
      (x, y) => {
        if (mask(x, y))
          image(x, y) * (1.0 - sor) + ((sor * 0.5) / (dxSq + dySq)) *: (dySq *: (image(x - 1, y) + image(x + 1,
                                                                                                         y
          )) + dxSq *: (image(x, y - 1) + image(x, y + 1)) - (dxSq * dySq) *: rhs(x, y))
        else
          f(x, y)
      },
      image.accessMode
  ...
  /**
   * perform a single red-black Gauss-Seidel relaxation of the Poisson equation: Laplace(f) = rhs Gauss-Seidel
   * relaxation uses updated values directly in the relaxation calculation, red-black performs two passes where the even
   * and odd are updated separately
   *
   * @param sor
   *   SOR (successive over-relaxation) constant use 0..1 to slow down, 1..2 to accelerate
   */
  def redBlackGaussSeidelRelaxation(f: PixelImage[A],
                                    mask: PixelImage[Boolean],
                                    rhs: PixelImage[A],
                                    hx: Double,
                                    hy: Double,
    ...
    val dxSq = hx * hx
    val dySq = hy * hy
    val oddFilter = ImageFilter((image: PixelImage[A]) =>
      PixelImage(
        image.width,
        image.height,
        (x, y) => {
    ...
    val evenFilter = ImageFilter((image: PixelImage[A]) =>
      PixelImage(
        image.width,
        image.height,
        (x, y) => {
  ...
  private def recursiveMultigridSolver(f: PixelImage[A],
                                       ...
                                       dx: Double,
                                       dy: Double,
                                       parameters: MultigridParameters
  ): PixelImage[A] = {
    ...
    if (
      width > parameters.minSize && height > parameters.minSize && restrictedMaskArea > 0.25 * parameters.minSize * parameters.minSize
    ) {
      ...
      val restrictedResidualMasked = PixelImage.fromTemplate(
        restrictedResidual,
        (x, y) => if (restrictedMask(x, y)) restrictedResidual(x, y) else ops.zero
      )
      ...
      val withProlongationCorrection = PixelImage(width,
                                                  height,
                                                  (x, y) => {
      ...
      val occlusionCorrected = solveSingleLevelPoisson(withProlongationCorrection,
                                                       occludedInLower,
                                                       rhs,
                                                       parameters.occlusionCorrectionIterations,
                                                       hx = dx,
                                                       hy = dy
      )
  ...
  private def restrict[B: ClassTag](image: PixelImage[B], w: Int, h: Int)(implicit
    ops: ColorSpaceOperations[B]
  ): PixelImage[B] = {
    ResampleFilter.resampleImage(image.withAccessMode(AccessMode.Repeat()), w, h, InterpolationKernel.BilinearKernel)
  }

  /**
   * pyramid / multigrid downsampling step: boundary mask, treat specially: convert to implicit representation which can
   * be interpolated and then convert back
   */
  ...
  private def prolongate[B: ClassTag](image: PixelImage[B], w: Int, h: Int)(implicit
    ops: ColorSpaceOperations[B]
  ): PixelImage[B] = {
    ResampleFilter.resampleImage(image.withAccessMode(AccessMode.Repeat()), w, h, InterpolationKernel.BilinearKernel)
  }
