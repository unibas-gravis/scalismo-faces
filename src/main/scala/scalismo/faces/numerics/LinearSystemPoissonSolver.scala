...
abstract class LinearSystemPoissonSolver[A](implicit vec: ComponentRepresentation[A])
  ...
  protected def findLinearization(mask: PixelImage[Boolean]): ImageLinearization = {
    ...
    var index = -1
    val reducedIndex = allPoints.map { case (x, y) =>
      if (mask(x, y)) {
        index += 1
        index
      } else
  ...
  protected def buildNegativeLaplaceMatrix(linearization: ImageLinearization,
                                           mask: PixelImage[Boolean],
                                           addElement: (Int, Int, Double) => Unit
  ): Unit = {
    linearization.order.foreach { case (x, y) =>
  ...
  protected def buildNegativeRHS(linearization: ImageLinearization,
                                 image: PixelImage[A],
                                 rhs: PixelImage[A],
                                 component: Int
  ): DenseVector[Double] = {
  ...
  protected def linearizeImage(linearization: ImageLinearization,
                               image: PixelImage[A],
                               component: Int
  ): DenseVector[Double] = {
    val n = linearization.order.size
  ...
  protected def reconstructImageComponent(solution: DenseVector[Double],
                                          linearization: ImageLinearization,
                                          image: PixelImage[A],
                                          component: Int
  ): PixelImage[Double] = {
    // reconstruct component image
    PixelImage(image.width,
               image.height,
               (x, y) => {
                 val rIndex = linearization.reducedIndex(x, y)
                 if (rIndex != -1)
...
/**
 * solve a Poisson equation in the image domain using a dense linear Cholesky solver (due to the dense matrix only for
 * small systems and a lot of memory)
 */
class DenseLinearSystemPoissonSolver[A: ClassTag](implicit vec: ComponentRepresentation[A])
...
/**
 * solve a Poisson equation using a sparse Cholesky decomposition (efficient for small systems, use PCG for larger
 * systems)
 */
class SparseCholeskyPoissonSolver[A: ClassTag](permutationStrategy: PermutationStrategy = PermutationStrategy.default)(
  implicit vec: ComponentRepresentation[A]
) extends LinearSystemPoissonSolver[A] {
...
/**
 * conjugate gradient solver for a Poisson system, no preconditioning (only for very small systems, prefer
 * Preconditioned CG)
 */
class ConjugateGradientPoissonSolver[A: ClassTag](tolerance: Double = 1e-6, maxIterations: Int = Int.MaxValue)(implicit
...
/**
 * conjugate gradient solver for a Poisson system, uses incomplete Cholesky preconditioning (considerably faster than
 * CG)
 */
class PreconditionedConjugateGradientPoissonSolver[A: ClassTag](tolerance: Double = 1e-6,
  ...
  override def solvePoisson(image: PixelImage[A], mask: PixelImage[Boolean], rhs: PixelImage[A]): PixelImage[A] = {
    ...
    val imageStack = for (c <- 0 until vec.size) yield {
      ...
      val xInitial = linearizeImage(linearization, image, c)
      // solve!
      val solution: DenseVector[Double] =
        PreconditionedConjugateGradient.solveSparse(A, b, xInitial, precond, tolerance, maxIter)
      // reconstruct component image
      reconstructImageComponent(solution, linearization, image, c)
    }
    // compose image stack
...
/**
 * conjugate gradient solver from breeze (for completeness, do not use, has no preconditioning and is slow, rather use
 * PreconditionedConjugateGradient and maybe ConjugateGradient)
 */
class BreezeCGPoissonSolver[A: ClassTag](tolerance: Double = 1e-6, maxIterations: Int = Int.MaxValue)(implicit
  vec: ComponentRepresentation[A]
) extends LinearSystemPoissonSolver[A] {
