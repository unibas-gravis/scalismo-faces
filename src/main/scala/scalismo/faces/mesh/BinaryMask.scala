...
import scalismo.geometry.{_3D, Dim, NDSpace}
...
/**
 * Mask is a function from a position in a sequence to a boolean. A mask can be used to remove the same position in many
 * sequences of values with the same length. This is usefull if the ordering has a semantical meaning, as for example in
 * the case of the point or triangle lists in a TriangleMesh3D.
 *
 * @param entries
...
case class BinaryMask(entries: IndexedSeq[Boolean]) extends (PointId => Boolean) {
   ...
   */
  def cut[D <: Dim: NDSpace: Create, DDomain[A] <: DiscreteDomain[A], T](
    field: DiscreteField[D, DDomain, T]
  )(implicit creator: UnstructuredPoints.Create[D]): DiscreteField[D, UnstructuredPointsDomain, T] = {
  ...
  def cut[T](iseq: IndexedSeq[T]): IndexedSeq[T] = {
    require(iseq.size == entries.size, "The IndexedSeq and the mask need to have the same length!!!")
  ...
  def cut(mask: BinaryMask): BinaryMask = {
    require(entries.size == mask.entries.size, "When masking a mask, the two masks need to have the same size!!!")
...
object BinaryMask {
   ...
   *
   * @param original
   *   Larger mesh of the two.
   * @param masked
   *   Mesh that can be made from the original when masking it.
   */
  ...
  /**
   * Creates a mask from an original and a masked UnstructuredPointsDomain. See
   * [[apply(TriangleMesh[_3D], TriangleMesh[_3D])]] for more information.
   *
   * @param original
   *   Larger UnstructuredPointsDomain.
   * @param masked
   *   Masked version of the larger UnstructuredPointsDomain.
   */
  def createFromUnstructuredPoints(original: UnstructuredPoints[_3D], masked: UnstructuredPoints[_3D]): BinaryMask = {
    ...
    require(
      maskedPoints.forall(p => maskedPoints.count(q => q == p) == 1 && originalPoints.count(q => q == p) == 1),
