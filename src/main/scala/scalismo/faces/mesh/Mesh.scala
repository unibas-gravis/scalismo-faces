 ...
 */
case class ColorNormalMesh3D(shape: TriangleMesh3D,
...
object ColorNormalMesh3D {
  def apply(vcMesh: VertexColorMesh3D): ColorNormalMesh3D =
    ColorNormalMesh3D(vcMesh.shape, vcMesh.color, vcMesh.shape.vertexNormals)
}
...
case class OptionalColorNormalMesh3D(shape: TriangleMesh3D,
  ...
  def toVertexColorMesh3D: Option[VertexColorMesh3D] = {
    color.map {
      ...
      case color: MeshSurfaceProperty[RGBA] =>
        VertexColorMesh3D(shape, SurfacePointProperty.averagedPointProperty(color))
    }
  }
...
object OptionalColorNormalMesh3D {
  def fromVertexColorMesh(vcMesh: VertexColorMesh3D): OptionalColorNormalMesh3D =
    OptionalColorNormalMesh3D(vcMesh.shape, Some(vcMesh.color), None)
  def fromTriangleMesh(triangleMesh3D: TriangleMesh3D): OptionalColorNormalMesh3D =
    OptionalColorNormalMesh3D(triangleMesh3D, None, None)
  def fromColorNormalMesh(cnMesh: ColorNormalMesh3D): OptionalColorNormalMesh3D =
    OptionalColorNormalMesh3D(cnMesh.shape, Some(cnMesh.color), Some(cnMesh.normals))
}
