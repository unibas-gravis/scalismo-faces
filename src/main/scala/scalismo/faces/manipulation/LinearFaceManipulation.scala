...
object LinearFaceManipulation {
  ...
  def manipulateShape(shape: IndexedSeq[Point[_3D]],
                      model: MoMo,
                      vector: MoMoCoefficients,
                      strength: Double
  ): IndexedSeq[Point[_3D]] = {
    require(shape.size == model.mean.shape.pointSet.numberOfPoints,
            "(shape) can only manipulate meshes compatible with the model"
    )
    ...
    // extract difference
    val diff =
      mean.shape.pointSet.points.zip(manipulatedMean.shape.pointSet.points).map { case (o, m) => m - o }.toIndexedSeq
    // apply to shape
    shape.zip(diff).map { case (s, d) => s + strength *: d }
  }

  /** linearly manipulate color, color sequence */
  def manipulateColor(color: IndexedSeq[RGBA],
                      model: MoMo,
                      vector: MoMoCoefficients,
                      strength: Double
  ): IndexedSeq[RGBA] = {
    require(color.size == model.mean.shape.pointSet.numberOfPoints,
            "(color) can only manipulate meshes compatible with the model"
    )
    // get manipulated mean
    val mean = model.mean
    val manipulatedMean = model.instance(vector)
    // extract difference
    val diff =
      for (i <- mean.color.triangulation.pointIds) yield manipulatedMean.color.atPoint(i) - mean.color.atPoint(i)
    // apply to color
    for (i <- color.indices) yield color(i) + strength *: diff(i)
  }

  /** linearly manipulate colored mesh, applies shape and color manipulation */
  def manipulateColorMesh(mesh: VertexColorMesh3D,
                          model: MoMo,
                          vector: MoMoCoefficients,
                          strengthShape: Double,
    ...
    VertexColorMesh3D(manipulateShapeMesh(mesh.shape, model, vector, strengthShape),
                      SurfacePointProperty(triangulation, manipulatedColor)
    )
  }

  /** linearly manipulate shape mesh */
  def manipulateShapeMesh(mesh: TriangleMesh3D,
                          model: MoMo,
                          vector: MoMoCoefficients,
                          strengthShape: Double
  ): TriangleMesh3D = {
    val triangulation = mesh.triangulation
    val manipulatedShape =
      manipulateShape(mesh.pointSet.points.map(p => p: Point[_3D]).toIndexedSeq, model, vector, strengthShape)
