...
object ModelHelpers {
   ...
   *
   * @param model
   *   DLRGP EuclideanVector[_3D] model
   * @param reference
   *   Reference used to map the deformation model to a point model.
   * @return
   *   DLRGP Point[_3D] model
   */
  def vectorToPointDLRGP(model: DiscreteLowRankGaussianProcess[_3D, TriangleMesh, EuclideanVector[_3D]],
    ...
    val newMeanField =
      vectorFieldToPointField(model.mean, (v: EuclideanVector[_3D], i: PointId) => reference.pointSet.point(i) + v)
   ...
   *   DLRGP Point[_3D] model
   * @param reference
   *   Reference used to map the point model to a deformation model.
   * @return
   *   DLRGP EuclideanVector[_3D] model
   */
  def pointToVectorDLRGP(model: DiscreteLowRankGaussianProcess[_3D, TriangleMesh, Point[_3D]],
                         reference: TriangleMesh[_3D]
  ): DiscreteLowRankGaussianProcess[_3D, TriangleMesh, EuclideanVector[_3D]] = {
    ...
    val newMeanField =
      pointFieldToVectorField(model.mean, (p: Point[_3D], i: PointId) => p - reference.pointSet.point(i))
   ...
   */
  def buildFrom[D <: Dim: NDSpace, DDomain[A] <: DiscreteDomain[A], Value](domain: DDomain[D],
                                                                           meanVec: DenseVector[Double],
                                                                           d2: DenseVector[Double],
  ...
  /**
   * Creates a discrete low rank GP from samples. This method assumes that the training data as DiscreteFields are in
   * dense correspondence with the DiscreteDomain as reference.
   *
   * @param domain
   *   The domain where the model is defined.
   * @param discreteFields
   *   The training samples.
   * @param threshold
   *   The minimal value to keep the basis.
   * @return
   *   A discrete low rank GP learned from the samples.
   */
  ...
  /**
   * Method to build a reconstructive model. The statistics is only calculated in the region indicated by the mask. Then
   * a full model is calculated based on the completion predicted by the input data.
   * @param domain
   *   domain of the full model
   * @param discreteFields
   *   data to build the full model from
   * @param mask
   *   indicates the region over which the statistic is calculated
   * @param threshold
   *   removes eigenvalue/eigenvector pairs if the eigenvalue is below the threshold
   */
  ...
  /**
   * Create a masked model using a set of PointIds. Please note that the procedure can return a model with less
   * PointId's because of the maskPoints operation.
   * @param momo
   *   The model to be masked.
   * @param pointIds
   *   The PointIds of the reference to be kept.
   * @return
   *   the masked model.
   */
  ...
  def maskMoMo(momo: MoMo, pointIds: Seq[PointId], strict: Boolean = true): Try[MoMo] = {
    ...
    if (momo.hasExpressions) {
      val maskedModelExpressions = {
        val newGP =
          momo.expressionModel.get.expression.gpModel.interpolate(NearestNeighborInterpolator()).discretize(newRef)
  ...
  /**
   * Create a masked model using a masked reference mesh. The input maskMesh for masking needs to be a subset of the
   * reference mesh.
   * @param momo
   *   The model to be masked.
   * @param maskMesh
   *   The masked mesh, which must be a subset of the original reference mesh. It will be the new reference mesh for the
   *   output MoMo.
   * @return
   *   the masked model.
   */
  ...
  def maskMoMo(momo: MoMo, maskMesh: TriangleMesh[_3D]): Try[MoMo] = {
    ...
    val remainingPtIds =
      maskMesh.pointSet.points.map(p => momo.referenceMesh.pointSet.findClosestPoint(p).id).toIndexedSeq
    ...
    if (maskMesh == maskedReference) {
      val newRef: TriangleMesh[_3D] =
        referenceMesh.operations.maskPoints(f => remainingPtIds.contains(f)).transformedMesh
   ...
   *   Data matrix.
   * @param noiseVariance
   *   External estimate of the noise variance.
   * @param threshold
   *   Threshold for keeping components.
   * @return
   *   (basis,variance,mean)
   */
   ...
   *   Data matrix.
   * @param M
   *   selection or rows to perform the PPCA on it
   * @param noiseVariance
   *   External estimate of the noise variance.
   * @param threshold
   *   Threshold for keeping components.
   ...
   *
   * @return
   *   The cols of the matrix hold the orthonormal eigenvectors and the vector the eigenvalues.
   */
  def decomposeGramMatrix(X: DenseMatrix[Double],
                          threshold: Double = 1.0e-8
