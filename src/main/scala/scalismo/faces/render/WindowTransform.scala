...
/**
 * transform a point from normalized coordinates [-1,1]x[-1,1]x[-1,1] to window coordinates [0,w]x[0,h]x[near,far] (y
 * still upwards!) used by renderer
 */
case class WindowTransform(width: Int, height: Int, near: Double = 0.0, far: Double = 1.0)
    extends InvertibleTransform3D {
...
case class InverseWindowTransform(width: Int, height: Int, near: Double = 0.0, far: Double = 1.0)
    extends InvertibleTransform3D {
...
/**
 * Use for rendering only the contents within a defined bounding box. transforms a point from normalized coordinates
 * [-1,1]x[-1,1]x[-1,1] to window coordinates [0,widthBox]x[0,heightBox]x[near,far] (y still upwards!)
 *
 * @param width
 *   of the full image
 * @param height
 ...
 * @param far
 */
case class WindowBoxTransform(width: Int,
                              height: Int,
                              widthBox: Int,
                              heightBox: Int,
                              posX: Int,
  ...
  /**
   * box scaled normalized coordinates go from [-1*ratioWidth/2 to 1*ratioWidth/2] boxScaledCoords = ((posX.toDouble /
   * width.toDouble)-0.5) * ratioWidth the box is located at (xInBoxScaledCoords, yInBoxScaleCoords)
   */
  ...
  override def inverted: InverseWindowBoxTransform =
    InverseWindowBoxTransform(width, height, widthBox, heightBox, posX, posY, near, far)
}

case class InverseWindowBoxTransform(width: Int,
                                     height: Int,
                                     widthBox: Int,
                                     heightBox: Int,
                                     posX: Int,
  ...
  override def inverted: WindowBoxTransform =
    WindowBoxTransform(width, height, widthBox, heightBox, posX, posY, near, far)
}
