  ...
  describe("Model building") {
    ...
    class Fixture {
      ...
      val trainingFields = trainingMeshes.map { mesh =>
        DiscreteField[_3D, Vector[_3D]](reference.shape.pointSet,mesh.shape.pointSet.points.zip(reference.shape.pointSet.points).map(a => a._2 - a._1).toIndexedSeq)
      }

      val scalismoModel = DiscreteLowRankGaussianProcess.createUsingPCA[_3D,Vector[_3D]](reference.shape.pointSet,trainingFields.map(_.interpolateNearestNeighbor()))
    ...
    it("a point model built from a deformation model should lead to the same shape given parameters") {
      ...
      val sample: DiscreteField[_3D, Vector[_3D]] = deformationModel.sample()
