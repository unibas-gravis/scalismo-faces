...
class DifferentialImageTest extends FacesTestSuite {
  ...
  describe("A PixelImage has proper differential operations") {
    ...
    it("has a proper gradient x, correlation with (-0.5, 0, 0.5)") {
      val testImg =
        PixelImage(ColumnMajorImageDomain(6, 1), (0 until 6).map(_.toFloat).toArray).withAccessMode(AccessMode.Repeat())
    ...
    it("has a proper gradient y, correlation with (-0.5, 0, 0.5)") {
      val testImg =
        PixelImage(ColumnMajorImageDomain(1, 6), (0 until 6).map(_.toFloat).toArray).withAccessMode(AccessMode.Repeat())
