...
class DistanceTransformTests extends FacesTestSuite {
  ...
  def stupidDistanceTransform(image: PixelImage[Boolean]): PixelImage[Double] = {
    PixelImage(
      ...
      image.height,
      (x, y) => {
        val distToEachImagePoint =
          PixelImage.view(image.width, image.height, (x1, y1) => EuclideanVector2D(x - x1, y - y1).norm)
        val maskedDistances =
          distToEachImagePoint.zip(image).mapLazy { case (dist, obj) => if (obj) dist else Double.PositiveInfinity }
  ...
  describe("In DistanceTransform") {
    ...
    val twoObjects =
      PixelImage(50,
                 50,
                 (x, y) => EuclideanVector2D(x - 25, y - 15).norm <= 5 || EuclideanVector2D(x - 45, y - 45).norm <= 10
      )
    ...
    describe("a signedDistanceTransform") {
      describe("of a simple image") {
        ...
        it("is correct outside") {
          ...
          assert(
            signedDistance.zip(distanceTransform).values.forall { case (signed, dist) =>
              if (dist > 0.0) signed == dist else true
            }
          )
        }
